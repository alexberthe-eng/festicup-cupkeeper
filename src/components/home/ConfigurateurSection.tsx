    ...
    <section className="py-14 lg:py-24 bg-background">
      <div className="container mx-auto px-4 lg:px-8">
        <div className="text-center mb-8">
          <h2 className="text-2xl lg:text-4xl font-serif font-bold mb-3">
            Votre évènement, nos gobelets
          </h2>
          <p className="text-muted-foreground max-w-xl mx-auto text-sm">
            Nous vous recommandons les produits adaptés à votre évènement, avec les quantités optimales.
          </p>
        </div>
        ...
        <div className="max-w-2xl mx-auto bg-background rounded-2xl shadow-lg border border-border p-5 lg:p-8">
          {/* Type d'événement */}
          <p className="text-xs font-medium text-muted-foreground mb-3">Type d'événement :</p>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 mb-6">
            {eventTypes.map((event) => (
              <button
                key={event.id}
                onClick={() => setSelectedEvent(event.id)}
                className={`relative rounded-xl overflow-hidden h-24 lg:h-28 group transition-all ${
              ...
              >
                ...
                <div className="relative z-10 flex items-center justify-center h-full px-2">
                  <span className="text-white text-[10px] lg:text-xs text-center font-medium leading-tight">
                    {event.label}
                  </span>
                </div>
              </button>
          ...
          {/* Nombre de personnes */}
          <p className="text-xs font-medium text-muted-foreground mb-3">Nombre de personnes :</p>
          <div className="flex items-center gap-3 mb-6">
            ...
            <input
              ...
              className="w-20 text-center text-xl font-bold bg-transparent border-none outline-none font-sans"
              min={25}
            />
            <button
          ...
          {/* CTA */}
          <Button className="w-full bg-foreground hover:bg-foreground/90 text-background rounded-lg h-11 text-sm font-medium">
            Continuer
          </Button>
        </div>
      </div>
